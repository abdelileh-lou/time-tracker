...
//         // Fetch employee data
//         const response = await axios.get(`http://localhost:8092/api/employee/${currentUser.id}`, { headers });
//         console.log("Fetched employee data:", response.data);

//         // Fetch PIN code
//         try {
//           // Make sure we're using the correct employee ID
//           const employeeId = response.data.id || currentUser.id;
//           console.log("Fetching PIN for employee ID:", employeeId); // Debug log

//           // Use the correct endpoint for code-pin
//           const pinResponse = await axios.get(`http://localhost:8092/api/employee/${employeeId}/code-pin`, { headers });
//           console.log("Fetched PIN code response:", pinResponse.data);

//           // Combine employee data with PIN code
//           const employeeData = {
//             ...response.data,
//             pinCode: pinResponse.data.pin
//           };

//           setEmployee(employeeData);
//         } catch (pinError) {
//           console.error("Error fetching PIN code:", pinError);
//           console.error("Error response:", pinError.response?.data); // Debug log

//           // Check specific error cases
//           if (pinError.response?.status === 403) {
//             setError("You don't have permission to view the PIN code. Please contact your administrator.");
//           } else if (pinError.response?.status === 404) {
//             setError("Employee not found.");
//           } else {
//             setError("Failed to load PIN code. Please try again later.");
//           }

//           // If PIN fetch fails, still set employee data but without PIN
