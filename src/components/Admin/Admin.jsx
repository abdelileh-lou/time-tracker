...
const Admin = () => {
  ...
  const [attendanceMethods, setAttendanceMethods] = useState({
    qrCode: { active: true, priority: 1 },
    facialRecognition: { active: true, priority: 2 },
    pinCode: { active: true, priority: 3 }, // Added PIN code method
  });
  ...
  const getHighestPriorityMethod = () => {
    const methodDisplayNames = {
      qrCode: "QR Code",
      facialRecognition: "Reconnaissance faciale",
      pinCode: "Code PIN", // Added display name for PIN code
    };
  ...
  const fetchMethodConfig = async () => {
    try {
      ...
      setAttendanceMethods({
        qrCode: { active: true, priority: 1 },
        facialRecognition: { active: true, priority: 2 },
        pinCode: { active: true, priority: 3 }, // Default value for PIN code
      });
    }
  };
  ...
  // Updated to handle multiple methods
  const handlePriorityChange = (method, newPriority) => {
    const currentMethods = { ...attendanceMethods };
    const oldPriority = currentMethods[method].priority;

    // Update all methods that need to change
    Object.keys(currentMethods).forEach((key) => {
      const methodConfig = currentMethods[key];

      if (key === method) {
        // Update the target method's priority
        methodConfig.priority = newPriority;
      } else if (methodConfig.priority === newPriority) {
        // Method that had the priority we're moving to gets the old priority
        methodConfig.priority = oldPriority;
      }
    });

    setAttendanceMethods(currentMethods);
  };
                          ...
                          <tbody>
                            {savedConfiguration.map((config, index) => (
                              <tr
                                ...
                                <td className="px-4 py-2">{config.priority}</td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
                  ...
                  <div className="bg-emerald-50 p-4 rounded-lg">
                    ...
                    <div className="space-y-4">
                      ...
                      <div>
                        ...
                        <input
                          type="email"
                          defaultValue="admin@ntic.example"
                          className="w-full px-4 py-2 border border-emerald-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent"
                        />
                      </div>
                      <div>
